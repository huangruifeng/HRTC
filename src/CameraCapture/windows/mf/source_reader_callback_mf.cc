  ...
  if (_control->IsRunning()) {
    ...
    if (pSample) {
      ...
      if (SUCCEEDED(hr)) {
        ...
        LONG pitch = 0;
        if (SUCCEEDED(hr) && _resultingCapability.format != hrtc::VideoPixelFormat::kMJPEG &&
            SUCCEEDED(mf2dBuffer->Lock2D(&pDatas, &pitch))) {
          mf2dBuffer->IsContiguousFormat(&isContinous);
          if (isContinous) {
            DWORD cLength;
            mf2dBuffer->GetContiguousLength(&cLength);
            length = CalcBufferSize(_resultingCapability.format, _resultingCapability.width, _resultingCapability.height);
        ...
        if (fallback) {
          ...
          if (SUCCEEDED(hr)) {
            if (_resultingCapability.format == VideoPixelFormat::kRGB24) {
              _resultingCapability.height = -_resultingCapability.height;
            }
            _control->PushIncomingFrame(pDatas, length, _resultingCapability);
            pBuffer->Unlock();
