...
int32_t DeviceInfoMF::NumberOfCapabilities(const std::string& deviceUniqueId) {
...
int32_t DeviceInfoMF::GetCapability(const std::string& deviceUniqueId,
...
int32_t DeviceInfoMF::CreateCapabilityMap(const std::string& deviceUniqueId) {
  ...
  IMFMediaSource* source = DeviceInfoMF::GetDeviceMediaSource(deviceUniqueId);
  if (!source)
    return -1;
  ...
  for (DWORD i = 0; i < count; i++) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(hr) && majorType == MFMediaType_Video) {
        VideoCapabilityWindowsMf capability;
        // get framesize
        UINT32 width = 0;
        UINT32 height = 0;
        ...
        if (subType == MFVideoFormat_I420) {
          ...
          capability.format = VideoPixelFormat::kNV12;
...
IMFMediaSource* DeviceInfoMF::GetDeviceMediaSource(
    const std::string& deviceUniqueId) {
  const size_t deviceUniqueIdLength = deviceUniqueId.size();  // UTF8 is also NULL terminated
  if (deviceUniqueIdLength > kVideoCaptureUniqueNameLength) {
    //LOG(LS_INFO) << "Device name too long";
    return NULL;
  }
  ...
  //LOG(LS_WARNING) << "Get filter by guid:" << deviceUniqueId;
  HRESULT hr = MFEnumDeviceSources(_mfAttribute, &ppDevices, &count);
  if (SUCCEEDED(hr) && deviceUniqueIdLength > 0 && ppDevices) {
    ...
    do {
      ...
      if (SUCCEEDED(hr)) {
        ...
        if (strncmp(tempDevicePathUTF8, deviceUniqueId.c_str(),
                    deviceUniqueIdLength) == 0) {
          //LOG(LS_INFO) << "we need found target guid:" << deviceUniqueId
