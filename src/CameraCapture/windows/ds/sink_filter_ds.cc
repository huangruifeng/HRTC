...
void GetSampleProperties(IMediaSample* sample, AM_SAMPLE2_PROPERTIES* props) {
  hrtc::rtc_refptr<IMediaSample2> sample2;
...
bool TranslateMediaTypeToVideoCaptureCapability(
  ...
  LOG_INFO("ds","TranslateMediaTypeToVideoCaptureCapability width:"
                   << bih->biWidth << " height:" << bih->biHeight
                   << " Compression:0x" << bih->biCompression);
...
class MediaTypesEnum : public IEnumMediaTypes {
 public:
  MediaTypesEnum(const VideoCapability& capability)
      : capability_(capability),
        format_preference_order_(
            {// Default preferences, sorted by cost-to-convert-to-i420.
             VideoPixelFormat::kI420, VideoPixelFormat::kYUY2, VideoPixelFormat::kRGB24,
             VideoPixelFormat::kUYVY, VideoPixelFormat::kMJPEG}) {
    // Use the preferred video type, if supported.
    auto it = std::find(format_preference_order_.begin(),
                        format_preference_order_.end(), capability_.format);
    if (it != format_preference_order_.end()) {
      LOG_INFO("ds","Selected video type: " << *it)
      ...
      LOG_WARNING("ds","Unsupported video type: "
                          <<static_cast<int>(capability_.format)
 ...
 private:
  ...
  static void SetMediaInfoFromVideoType(VideoPixelFormat video_type,
  ...
  int pos_ = 0;
  const VideoCapability capability_;
  std::list<VideoPixelFormat> format_preference_order_;
};
...
HRESULT CaptureInputPin::SetRequestedCapability(
  ...
  resulting_capability_ = VideoCapability();
  return S_OK;
}
...
CaptureInputPin::QueryAccept(const AM_MEDIA_TYPE* media_type) {
  VideoCapability capability(resulting_capability_);
...
CaptureSinkFilter::CaptureSinkFilter(VideoDeviceBase* capture_observer)
...
CaptureSinkFilter::JoinFilterGraph(IFilterGraph* graph, LPCWSTR name) {
  ...
  if (info_.pGraph) {
    ...
    hrtc::rtc_refptr<IMediaEventSink> sink;
...
void CaptureSinkFilter::ProcessCapturedFrame(
    ...
    size_t length,
    const VideoCapability& frame_info) {
  // Called on the capture thread.
  capture_observer_->DeliverFrame(buffer, length, frame_info);
}
