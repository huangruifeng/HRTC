...
int32_t DeviceInfoDS::Init() {
  ...
  if (hr != NOERROR) {
    LOG_INFO("ds","Failed to create CLSID_SystemDeviceEnum, error 0x"
                     << hr);
    return -1;
  }
  return 0;
...
int32_t DeviceInfoDS::GetDeviceName(uint32_t deviceNumber,
  ...
  std::lock_guard<std::mutex> lock(_apiLock);
  char nameUTF8[kVideoCaptureDeviceNameLength] = {0};
  char uniqueIdUTF8[kVideoCaptureUniqueNameLength] = {0};
  const int32_t result = GetDeviceInfo(deviceNumber, nameUTF8, sizeof(nameUTF8),
                                       uniqueIdUTF8, sizeof(uniqueIdUTF8), 0, 0);
  if (result > static_cast<int32_t>(deviceNumber)) {
    deviceName = nameUTF8;
    deviceUniqueId = uniqueIdUTF8;
    return 0;
  }
  return -1;
}

int32_t DeviceInfoDS::NumberOfCapabilities(const std::string& deviceUniqueId) {
  ...
  // 同一设备再次查询时直接返回缓存
  if (_lastUsedDeviceName == deviceUniqueId) {
    return static_cast<int32_t>(_captureCapabilities.size());
  }

  return CreateCapabilityMap(deviceUniqueId);
}

int32_t DeviceInfoDS::GetCapability(const std::string& deviceUniqueId,
                                    const uint32_t deviceCapabilityNumber,
                                    VideoCapability& capability) {
  std::lock_guard<std::mutex> lock(_apiLock);

  if (_lastUsedDeviceName != deviceUniqueId) {
    if (-1 == CreateCapabilityMap(deviceUniqueId)) {
      return -1;
    }
  }
...

{
  ...
  HRESULT hr = _dsDevEnum->CreateClassEnumerator(CLSID_VideoInputDeviceCategory,
                                                 &_dsMonikerDevEnum, 0);
  if (hr != NOERROR) {
    LOG_INFO("ds","Failed to enumerate CLSID_SystemDeviceEnum, error 0x"
                     << hr << ". No webcam exist?")
    return 0;
  }
        ...
        if ((wcsstr(varName.bstrVal, (L"(VFW)")) == NULL) &&
          ...
          if (index == static_cast<int>(deviceNumber)) {
            ...
            if (deviceNameLength > 0) {
              ...
              if (convResult == 0) {
                LOG_INFO("ds","Failed to convert device name to UTF8, "
                                    "error = "
                                 << GetLastError())
                return -1;
              }
            ...
            if (deviceUniqueIdUTF8Length > 0) {
              ...
              if (FAILED(hr)) {
                ...
                if (convResult == 0) {
                  LOG_INFO("ds","Failed to convert device "
                                      "name to UTF8, error = "
                                   << GetLastError())
                  return -1;
                }
  ...
  if (deviceNameLength) {
    LOG_INFO("ds",__FUNCTION__ << " " << deviceNameUTF8)
  }
  return index;
}
...
IBaseFilter* DeviceInfoDS::GetDeviceFilter(const char* deviceUniqueIdUTF8,
  ...
  if (deviceUniqueIdUTF8Length >= kVideoCaptureUniqueNameLength) {
    LOG_ERROR("ds","Device name too long")
    return NULL;
  }
  ...
  HRESULT hr = _dsDevEnum->CreateClassEnumerator(CLSID_VideoInputDeviceCategory,
                                                 &_dsMonikerDevEnum, 0);
  if (hr != NOERROR) {
    LOG_ERROR("ds","Failed to enumerate CLSID_SystemDeviceEnum, error 0x"
                     << hr << ". No webcam exist?")
    return 0;
  }
  _dsMonikerDevEnum->Reset();
      ...
      if (deviceUniqueIdUTF8Length > 0) {
        ...
        if (SUCCEEDED(hr)) {
          ...
          if (strncmp(tempDevicePathUTF8, (const char*)deviceUniqueIdUTF8,
            ...
            if FAILED (hr) {
              LOG_ERROR("ds","Failed to bind to the selected "
                                   "capture device "
                                << hr)
            }
...
int32_t DeviceInfoDS::CreateCapabilityMap(const std::string& deviceUniqueId)

{
  // Reset old capability list
  _captureCapabilities.clear();

  const size_t deviceUniqueIdLength = deviceUniqueId.size();
  if (deviceUniqueIdLength >= kVideoCaptureUniqueNameLength) {
    LOG_INFO("ds","Device name too long")
    return -1;
  }
  LOG_INFO("ds","CreateCapabilityMap called for device "
                   << deviceUniqueId);

  char productId[kVideoCaptureProductIdLength];
  IBaseFilter* captureDevice = DeviceInfoDS::GetDeviceFilter(
      deviceUniqueId.c_str(), productId, kVideoCaptureProductIdLength);
  if (!captureDevice)
    return -1;
  IPin* outputCapturePin = GetOutputPin(captureDevice, GUID_NULL);
  if (!outputCapturePin) {
    LOG_INFO("ds","Failed to get capture device output pin";
  ...
  if (SUCCEEDED(hr) && extDevice) {
    LOG_INFO("ds","This is an external device")
    extDevice->Release();
  }
  ...
  hr = outputCapturePin->QueryInterface(IID_IAMStreamConfig,
                                        (void**)&streamConfig);
  if (FAILED(hr)) {
    LOG_INFO("ds","Failed to get IID_IAMStreamConfig interface "
                        "from capture device")
    return -1;
  }
  ...
  if (FAILED(hrVC)) {
    LOG_INFO("ds","IID_IAMVideoControl Interface NOT SUPPORTED")
  }
  ...
  if (FAILED(hr)) {
    LOG_INFO("ds","Failed to GetNumberOfCapabilities")
  ...
  for (int32_t tmp = 0; tmp < count; ++tmp) {
    ...
    if (hr == S_OK) {
      if (pmt->majortype == MEDIATYPE_Video &&
        ...
        LOG_INFO("ds","Device support FORMAT_VideoInfo2")
  ...
  for (int32_t tmp = 0; tmp < count; ++tmp) {
    hr = streamConfig->GetStreamCaps(tmp, &pmt, reinterpret_cast<BYTE*>(&caps));
    if (hr != S_OK) {
      LOG_INFO("ds","Failed to GetStreamCaps")
      RELEASE_AND_CLEAR(videoControlConfig);
      RELEASE_AND_CLEAR(streamConfig);
      RELEASE_AND_CLEAR(outputCapturePin);
      RELEASE_AND_CLEAR(captureDevice);
    ...
    if (pmt->majortype == MEDIATYPE_Video &&
      ...
      VideoCapabilityWindows capability;
      ...
      if (hrVC == S_OK) {
        ...
        {
          LOG_INFO("ds","GetMaxAvailableFrameRate NOT SUPPORTED")
      ...
      if (pmt->subtype == MEDIASUBTYPE_I420) {
        capability.format = VideoPixelFormat::kI420;
      } else if (pmt->subtype == MEDIASUBTYPE_IYUV) {
        capability.format = VideoPixelFormat::kIYUV;
      } else if (pmt->subtype == MEDIASUBTYPE_RGB24) {
        capability.format = VideoPixelFormat::kRGB24;
      } else if (pmt->subtype == MEDIASUBTYPE_YUY2) {
        capability.format = VideoPixelFormat::kYUY2;
      } else if (pmt->subtype == MEDIASUBTYPE_RGB565) {
        capability.format = VideoPixelFormat::kRGB565;
      } else if (pmt->subtype == MEDIASUBTYPE_MJPG) {
        capability.format = VideoPixelFormat::kMJPEG;
      ...
      {
        capability.format =
            VideoPixelFormat::kYUY2;  // MS DV filter seems to create this type
      } else if (pmt->subtype ==
                 MEDIASUBTYPE_UYVY)  // Seen used by Declink capture cards
      {
        capability.format = VideoPixelFormat::kUYVY;
      } else if (pmt->subtype ==
      ...
      {
        LOG_INFO("ds","Device support HDYC.")
        capability.format = VideoPixelFormat::kUYVY;
      } else {
        WCHAR strGuid[39];
        StringFromGUID2(pmt->subtype, strGuid, 39);
        LOG_WARNING("ds","Device support unknown media type " << strGuid << ", width "
      ...
      LOG_INFO("ds","Camera capability, width:" << capability.width
                       << " height:" << capability.height
                       << " type:" << static_cast<int>(capability.format)
  ...
  _lastUsedDeviceName = deviceUniqueId;
  LOG_INFO("ds","CreateCapabilityMap " << _captureCapabilities.size());
...
void DeviceInfoDS::GetProductId(const char* devicePath,
  ...
  *productUniqueIdUTF8 = '\0';
  ...
  if (!startPos) {
    ...
    LOG_INFO("ds","Failed to get the product Id")
    return;
  }
  startPos += 4;
