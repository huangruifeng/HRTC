...
class CaptureInputPin : public IMemInputPin, public IPin {
 public:
  ...
  HRESULT SetRequestedCapability(const VideoCapability& capability);
 ...
 private:
  ...
  VideoCapability requested_capability_ ;
  // Accessed on the main thread when Filter()->IsStopped() (capture thread not
  // running), otherwise accessed on the capture thread.
  VideoCapability resulting_capability_;
  DWORD capture_thread_id_ = 0;
  hrtc::rtc_refptr<IMemAllocator> allocator_ ;
  hrtc::rtc_refptr<IPin> receive_pin_ ;
...
class CaptureSinkFilter : public IBaseFilter {
 public:
  CaptureSinkFilter(VideoDeviceBase* capture_observer);
 ...
 private:
  const hrtc::rtc_refptr<ComRefCount<CaptureInputPin>> input_pin_;
  VideoDeviceBase* const capture_observer_;
  FILTER_INFO info_  = {};
