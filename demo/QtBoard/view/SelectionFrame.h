...
// 选择框：围绕目标元素集合的虚线外接矩形（并集）+ 8 个缩放手柄 + 顶部旋转手柄
// + 右上角删除按钮。位置/形状随所有目标 item 的变换实时同步（场景坐标）。
...
public:
    ...
    static constexpr qreal kDeleteOffset = 16;   // 删除按钮距右上角偏移（斜向外移）
    static constexpr qreal kDeleteRadius = 10;
...
private:
    ...
    QVector<QPointF> handlePos_;   // Handle 枚举序，手柄中心（场景坐标）
    QVector<QRectF> handleRect_;   // 命中矩形（旋转/删除手柄为圆，用外接方形近似）
